...
## Campaign Edition v6 — Astro Static Build (supersedes v1–v5)
...
- **Preloader:** the thread draws the Rizvi wordmark (SVG stroke, ≤1.8s, session-cached via sessionStorage, never repeats in-session), then releases into the page.
...
**Page transitions.** A fabric-wipe between pages — black panel sweep while the thread re-draws into the next page's rail — via Astro's built-in View Transitions API; reduced-motion gets an instant cut. **Lifecycle law (HARD):** on every page navigation every page-scoped instance — ScrollTrigger triggers, SplitText splits, Lenis bindings, observers, event listeners — must be explicitly destroyed on `astro:before-swap` and rebuilt on `astro:page-load`; memory must not grow materially across twenty consecutive navigations (verified in QA). Time-boxed; a hard cut is always acceptable.
...
1. It's an **Astro static site**, not a WordPress theme.
2. Build a **deployable `/dist` folder**, not a CMS-dependent deliverable — the final output must be uploadable to cPanel's `public_html` as plain files with zero server-side dependencies.
3. Produce `DEPLOY.md` listing every manual step to go live: build command, what to upload, any `.htaccess` rules needed for clean URLs, and a "full cPanel backup before deploying" warning.
4. **SEO continuity is owned by the Astro build itself** — preserve all production slugs, ensure every page has a unique `<title>` and `<meta description>`, one `<h1>` per page, descriptive `alt` everywhere, canonical tags, and a generated `sitemap.xml`. Nothing `noindex`ed.
5. Generate `README.md` with instructions on how to build and deploy this site, and how to run it locally for preview.

---

# 11. Content system architecture

- **Total content/structure separation.** No copy in component markup. All text, labels, stats, people, partners, image references, and per-page section orders live in `src/data/*.json` (or `.ts` typed data files) with self-explanatory keys and comments; components render the data layer.
...
- **v2 path:** `HANDOFF.md` documents migrating the data layer to a headless CMS (e.g. Contentful, Sanity) later if the owner wants browser-based editing.

---

# 12. Asset & cutout pipeline

1. Fetch Drive → `reference/drive/`; server uploads per §8. View every image; write `src/data/image-map.json` assigning images to slots by subject.
...
4. Responsive derivatives: WebP at 1920/1280/768/480 with Astro's `<Image />` component and `srcset`; portraits and partner logos staged from live-site reference (logos re-toned white/grey for dark ground; tonal adjustment yes, redrawing no).

---

# 13. Motion & interaction loadout

- **Library selection (G3 decision).** The motion stack is chosen at G3 after studying the inspiration sites. The sanctioned candidates are: **GSAP + ScrollTrigger + SplitText** (gold standard for scroll choreography), **Lenis** (smooth scroll), and **Motion** (formerly Framer Motion's vanilla JS library — for component-level declarative animation). Claude Code selects the combination that best replicates the inspiration site patterns, documents the decision in `DECISIONS.md`, and pins all versions. All libraries bundled locally; licences verified at install and logged in `CREDITS.md`.
- **Page transitions** use **Astro's built-in View Transitions API** (`<ViewTransitions />` in the base layout) — fabric-wipe implemented as a custom `transition:animate` definition; reduced-motion gets an instant cut. Barba.js is not used (redundant with Astro's native solution).
...
- **Lifecycle law (HARD):** every page-scoped instance — ScrollTrigger triggers, SplitText splits, Lenis bindings, observers, event listeners — must be explicitly destroyed in `astro:before-swap` and rebuilt in `astro:page-load`; no listener, trigger, or split survives a page it belongs to; twenty-navigation memory check in QA.
- **Reduced motion (HARD):** everything collapses — static rail, fades, instant transitions, vertical product list. Honour `prefers-reduced-motion` in both CSS and JS.
- **Feature flags:** every Track B/C element independently switchable in one config file (`src/config/flags.ts`), documented in `HANDOFF.md`.
...
11. **Contact:** structure preserved (HARD), recoloured and re-typeset; the form is a real, functional HTML form — use **Netlify Forms** syntax (`netlify` attribute) as the default handler, with a fallback `action` pointing to a `mailto:` for pure static hosts; documented in `DEPLOY.md` so the owner knows what to enable on their host.
...
13. **404:** minimal, on-system (`404.astro`), thread present.

---

# 15. Astro engineering requirements

- **Framework:** Astro (latest stable), configured for **static output** (`output: 'static'` in `astro.config.mjs`). No SSR, no server endpoints. The build command is `npm run build`; output lands in `/dist`.
- **Project structure:** `src/pages/` (one `.astro` file per route, file names matching production slugs), `src/layouts/` (base layout with `<ViewTransitions />`, thread rail, nav, footer), `src/components/`, `src/data/` (all content JSON/TS), `src/styles/` (global tokens CSS), `src/scripts/` (motion and interaction JS), `public/` (fonts, static images that bypass the asset pipeline), `src/assets/` (images processed by Astro's `<Image />`).
- **Routing:** file-based; slug mapping is direct — `src/pages/overview.astro` → `/overview/`, `src/pages/company-profile.astro` → `/company-profile/`, etc. All production slugs preserved (HARD). Trailing-slash behaviour set consistently (`trailingSlash: 'always'` or `'never'` — decide once, log it, add matching `.htaccess` rule in `DEPLOY.md`).
- **SEO:** Astro's `@astrojs/sitemap` integration generates `sitemap.xml` at build time. Every page sets unique `<title>`, `<meta name="description">`, canonical `<link>`, and Open Graph tags via the base layout's props. One `<h1>` per page; descriptive `alt` everywhere; nothing `noindex`ed.
- **Integrations:** `@astrojs/sitemap`, `@astrojs/tailwind` (or vanilla CSS with the token system — choose at G3 and log it). No React/Vue/Svelte island framework unless a specific interactive component genuinely requires it; prefer vanilla JS + Astro's `<script>` tags.
- **Asset pipeline:** use Astro's built-in `<Image />` and `<Picture />` for responsive images with WebP conversion and `srcset`. Self-hosted fonts loaded via `@font-face` in global CSS, `font-display: swap`, preloaded in `<head>`.
...
- **Clean URLs on cPanel:** `DEPLOY.md` includes the `.htaccess` snippet needed to serve `/overview/index.html` at `/overview/` on Apache shared hosting.
- **No build-time secrets or server-side logic of any kind.** The output is inert static files.
...
**Day 1 — Intelligence & design lock.** G0 scaffold (workspace, git, CLAUDE.md, `.claude/` settings + agent roster: recon, curator, site-engineer, motion-engineer, fresh-context critic). G1 recon (crawl live site, investigate counter values, confirm all slugs and anchors, extract pink hex, download portraits/logos). G2 assets + cutouts + cutout QA. G3 design lock: `INSPIRATION.md`, type bake-off, **motion stack selection** (GSAP/ScrollTrigger/SplitText + Lenis + Motion — confirm combination and versions), nav primary-set confirmation, contact form handler decision, trailing-slash decision, `DESIGN.md` with the complete token system, thread spec, hero storyboard, per-page wireframes; token-lint built; tokens into CLAUDE.md.

**Day 2 — Track A (must ship).** G4 Astro project initialised, integrations installed, base layout (top bar, overlay menu, thread rail, footer) + data layer + all pages populated with seed content; `npm run build` succeeds; `npm run preview` serves all pages; swap test passes; token-lint clean; zero console errors. G5 Track A QA to thresholds; SEO smoke test (sitemap renders, titles unique, one h1 per page, alts present); budgets met. *Track A alone is a respectable deliverable.*

**Day 3 — Track B.** G6 motion layer: View Transitions fabric-wipe, thread rail + seams, hero weave, SplitText reveals, horizontal gallery, Lenis smooth scroll **with full lifecycle teardown verified on `astro:before-swap` / `astro:page-load`**, micro-interactions; rubric re-passed; reduced-motion verified; FPS traced; twenty-navigation memory check passed.
...
**Day 5 — Release.** G9 `dist/` clean build verified — all pages render, no broken links, sitemap present, `.htaccess` included. G10 full documentation set (§19) + `FINAL_REPORT.md`.
...
- **Local preview:** `npm run dev` (Astro dev server) for development; `npm run build && npm run preview` to verify the production build locally before any QA pass. Both must work from a clean `npm install`.
...
- **Build gate:** `npm run build` must exit 0 with zero errors or warnings before any QA pass is counted.
- **Broken-link check:** run a local link crawler against `npm run preview`; zero 404s on internal links.
- **SEO smoke test:** sitemap.xml present and parseable; every page has unique title and meta description; one h1 per page; no noindex; canonical tags present.
